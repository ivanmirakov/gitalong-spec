...
!!! note "Note on salting"
    To prevent hash collisions and ensure privacy, all `content_blob` payloads **MUST** include a `salt` field with random data. This ensures that identical messages result in unique hashes.
...
A chat room for example, will have a state tree (what the room is, who joined, who is banned), this is the *main* tree. In a chat room, it will point to another tree, the **timeline tree** (the messages). Since the timeline tree is not a main tree, every node needs to hold a reference of the latest state node that the author knew of when appending it.
